...
impl<'a> MacroAnalyzer<'a> {
    ...
    fn infer_type_from_tokens(
        ...
        // メンバアクセス式 (x)->field の場合、フィールドの型を優先
        // （キャスト型より具体的なため）
    ...
    fn infer_from_member_access(&self, tokens: &[Token], _def: &MacroDef) -> Option<String> {
        // (param)->field または param.field パターンを探す
        // 最後のメンバアクセスを使う（式の最終的な結果を表すため）
        let mut last_field: Option<InternedStr> = None;

        for (i, token) in tokens.iter().enumerate() {
            // -> または . の後の識別子を取得
            let is_member_access = matches!(token.kind, TokenKind::Arrow | TokenKind::Dot);
            if is_member_access {
                if i + 1 < tokens.len() {
                    if let TokenKind::Ident(field_name) = tokens[i + 1].kind {
                        last_field = Some(field_name);
                    }
                }
            }
        }
