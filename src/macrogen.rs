...
pub fn generate(config: &MacrogenConfig) -> Result<MacrogenResult, MacrogenError> {
    ...
    // 10. ヘッダーを出力
    writeln!(output, "// Auto-generated Rust functions from C macros and inline functions")?;
    writeln!(output, "// Generated by libperl-macrogen")?;
    writeln!(output)?;
    writeln!(output, "#![allow(non_snake_case)]")?;
    writeln!(output, "#![allow(unused)]")?;
    writeln!(output)?;
    writeln!(output, "use std::ffi::{{c_char, c_int, c_uint, c_long, c_ulong, c_void}};")?;
    writeln!(output)?;

    // 11. inline関数を出力
    if config.include_inline_functions {
        writeln!(output, "// ==================== Inline Functions ====================")?;
        writeln!(output)?;
    ...
    if config.include_macro_functions {
        ...
        // 14. マクロ関数を出力
        writeln!(output)?;
        writeln!(output, "// ==================== Macro Functions ====================")?;
        writeln!(
            output,
        ...
        for name in macro_names {
            ...
            if let Some(reason) = parse_failure_map.get(&name_str) {
                writeln!(output, "// FAILED: {} - {}", name_str, reason)?;
