...
- typedef 経由の anonymous union（`typedef union { ... } NAME_aligned;` 系）は
  名前付き struct とは別経路で、現在は捕捉していない（要対応）
- 関数ポインタフィールドは `to_rust_string` が `/* fn */` を返すため
  `[SKIPPED]` 扱い
- `static const struct X[] = {...}` の初期化リスト → Rust `static` への
  翻訳は未実装（`bodies_by_type` 等。Phase 2 として別タスク化）
