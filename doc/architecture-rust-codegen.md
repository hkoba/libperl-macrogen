...
C AST → syn::Expr 直接構築 → parenthesize → 整形 のフローへ移行済み。
...
- Phase 5: ✅ `build_syn_expr` を主経路として完成
  ...
  全マクロ式・文・inline 関数が `build_syn_expr` を経由し、
  `--use-syn-expr` フラグは廃止された。
  整数幅キャスト・bool 変換・ポインタオフセット等も全て syn::Expr 上で
  挿入されるため、文字列レベルの "as type" 優先順位崩壊は発生しない。

残存する文字列パス (`expr_to_rust_ctx` / `expr_to_rust_inline_ctx` 等)
は次の経路から呼ばれるのみ:

- `try_expand_call_as_lvalue{,_inline}`: lvalue マクロ展開時の本体生成
- `build_syn_expr` の `StmtExpr` / catch-all `_` arm のフォールバック
- `offsetof` 内 TypeName 引数の文字列化

これらの呼び出しを `build_syn_expr` 経由に置換すれば、文字列パス全体を削除できる
（フォロー作業）。
