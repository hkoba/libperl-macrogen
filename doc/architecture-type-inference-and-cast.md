...
### `&` 演算子は `&raw mut` で生ポインタを生成

`ExprKind::AddrOf` は `syn_codegen::addr_of_mut` を経由して
**`&raw mut <expr>`**（`syn::Expr::RawAddr`、Rust 2024 安定化）を出力する。

C の `&x` は本来「アドレスを取る = 生ポインタを得る」操作で、Rust の
`&mut T` 参照とは意味論が異なる。FFI 中心の生成コードでは生ポインタが
正解で、また `(SV**)&sv` のような C キャストパターンが
`&raw mut sv as *mut *mut SV` に翻訳されるため、`&mut x as *mut T`
（参照→生ポインタの直接 cast）の E0606 が回避される。
